...
// ==== BAGONG: heartbeat/silent-death detection ====
// Sinusubaybayan ang huling pagkakataon na may natanggap na EVENT (kahit anong
// type) mula sa listener ng bawat account. Kung matagal nang walang natatanggap
// na event (default 20 minuto), maaaring "silent death" na ito — patay na ang
// koneksyon pero hindi ito na-detect ng library bilang error/close. Ipe-force
// ang reconnect sa ganitong sitwasyon.
const lastActivity = new Map();
const HEARTBEAT_CHECK_INTERVAL_MS = 5 * 60 * 1000; // kada 5 minuto susuriin
const SILENT_DEATH_THRESHOLD_MS = 20 * 60 * 1000; // 20 minutong walang event = considered patay
const activeSessions = new Map(); // userid -> { enableCommands, prefix, admin }
    ...
    login({
      ...
      try {
        var listenEmitter = api.listenMqtt(async (error, event) => {
          // BAGONG: i-update ang timestamp kada may natanggap na kahit anong
          // event, buhay man o may laman man ito — ito ang basehan ng
          // heartbeat check para ma-detect ang "silent death"
        ...
        // BAGONG: i-rehistro ang session na 'to sa activeSessions, at i-set
        // ang unang lastActivity timestamp — kailangan ito ng heartbeat
        // checker function para malaman kung sino ang susubaybayan
        activeSessions.set(userid, { enableCommands, prefix, admin });
        lastActivity.set(userid, Date.now());
...
// ==== BAGONG FUNCTION: heartbeat checker para sa "silent death" detection ====
// Tumatakbo kada HEARTBEAT_CHECK_INTERVAL_MS (default 5 min). Sinusuri ang
// bawat active session — kung matagal nang walang natatanggap na kahit anong
// event (default 20 min), ipinapalagay na patay na ang koneksyon kahit hindi
// ito na-detect ng ws3-fca library bilang error/close, at ipinipilit ang
// reconnect.
function startHeartbeatChecker() {
  setInterval(() => {
    ...
    for (const [userid, sessionInfo] of activeSessions.entries()) {
      ...
      if (idleTime > SILENT_DEATH_THRESHOLD_MS) {
        console.warn(
          `[heartbeat] Walang natanggap na event mula kay ${userid} sa loob ng ` +
          `${Math.round(idleTime / 60000)} minuto. Posibleng silent death — ` +
          `ipinipilit ang reconnect.`
        );
        activeSessions.delete(userid);
        lastActivity.delete(userid);
        attemptReconnect(userid, sessionInfo.enableCommands, sessionInfo.prefix, sessionInfo.admin);
      }
    }
  }, HEARTBEAT_CHECK_INTERVAL_MS);
  console.log(
    `[heartbeat] Sinimulan ang heartbeat checker — susuriin kada ` +
    `${HEARTBEAT_CHECK_INTERVAL_MS / 60000} min, considered patay pagkatapos ` +
    `ng ${SILENT_DEATH_THRESHOLD_MS / 60000} min na walang activity.`
  );
}
